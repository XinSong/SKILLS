...
5. 普通自动裁剪只接受覆盖原帧至少 55%、且至少有三条独立检测页面边缘的候选
   矩形。对于贴在左侧或右侧、覆盖 40%–55% 的分屏课件，还要求完整上下边界；
   对于四边都位于广播画面内部、覆盖 50%–55% 的内嵌课件，则要求课件靠近画面
   一个角、检测到四条页面边缘和四侧暗色外部证据。两种低面积路径都必须经过
   全分辨率确认，并在至少三个时间连续的稳定状态中重复出现相同裁切范围。这样
   既支持“课件 + 演讲者”和“课件 + 会场条幅”版式，也避免
   把原生全屏课件内部的标题框、图表或示意图误裁成整页；不确定时保留完整原帧
   供复核。
